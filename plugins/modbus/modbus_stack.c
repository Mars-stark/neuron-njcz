...
//stack: 指向 Modbus 协议栈实例的指针，这个栈管理与 Modbus 设备的通信。
// slave_id: Modbus 从机的 ID，用于标识要与之通信的特定设备。
// area: 要读取的数据区域，例如输入寄存器（MODBUS_AREA_INPUT_REGISTER）、保持寄存器（MODBUS_AREA_HOLD_REGISTER）等。
// start_address: 要读取的寄存器的起始地址。
// n_reg: 要读取的寄存器数量。
// response_size: 指向 uint16_t 类型的指针，用于存储响应数据的大小。
...
{   
    ...
    area=MODBUS_AREA_HOLD_REGISTER;
